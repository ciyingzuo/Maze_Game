...
class Node {
    ...
    void draw(WorldScene bg) {
        ...
        bg.placeImageXY(new RectangleImage(MazeWorld.EDGEWIDTH,
                MazeWorld.EDGEHEIGHT, "solid", c), p.x, p.y);
    }
...
class Edge {
    ...
    void draw(WorldScene bg) {
        ...
        if (a.x == b.x) {
            bg.placeImageXY(new RectangleImage(MazeWorld.EDGEWIDTH,
                    thickness, "solid", c), p.x, p.y);
        }
        else {
            bg.placeImageXY(new RectangleImage(thickness,
                    MazeWorld.EDGEHEIGHT, "solid", c), p.x, p.y);
        }
    }
}
...
class ExamplesMazeWorld {
    ...
    Posn n4Posn = new Posn(MazeWorld.EDGEWIDTH / 2,
    ...
    void testUnionFind(Tester t) {
        ...
        //  union the nodes
        MazeUtils.union(hash, MazeUtils.find(hash, n1.toString()),
                MazeUtils.find(hash, n2.toString()));
        t.checkExpect(MazeUtils.find(hash, n1.toString()), n2.toString());
        //  union the nodes
        MazeUtils.union(hash, MazeUtils.find(hash, n1.toString()),
                MazeUtils.find(hash, n3.toString()));
